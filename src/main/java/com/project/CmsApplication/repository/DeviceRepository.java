...
public interface DeviceRepository extends JpaRepository<Device, Integer> {
    ...
    @Query(value = "SELECT * from cms_2.Device WHERE " +
    ...
    List<Device> getDeviceList(@Param("branch_id") String branch_id,
...
//    @Modifying
//    @Query(value = "INSERT INTO cms_2.Device(status,device_name,created_by,created_date,updated_by,updated_date) " +
...
//    void save(@Param("device_name") String device_name, @Param("created_by") String created_by);
    ...
    @Query(value = "SELECT * from cms_2.Device WHERE device_id =:device_id", nativeQuery = true)
    List<Device> getDeviceById(@Param("device_id") int device_id);

//    @Query(value = "SELECT * from cms_2.Device WHERE license_key =:license_key", nativeQuery = true)
//    List<Device> getDeviceByLicenseKey(@Param("license_key") String license_key);

    @Query(value = "SELECT * from cms_2.Device WHERE license_key =:license_key AND device_unique_id = '' and status <> 'deleted'", nativeQuery = true)
    ...
    @Modifying
    @Query(value = "UPDATE cms_2.Device SET status = 'deleted',updated_by=:updated_by," +
    ...
    @Query(value = "SELECT * from cms_2.device WHERE device_id in (" +
            "SELECT distinct(device_id) from cms_2.position " +
            "WHERE position_id in " +
            "    (SELECT distinct(position_id) from cms_2.playlist WHERE CAST(company_id AS VARCHAR) like :company_id AND ((CAST(branch_id AS VARCHAR) like :branch_id AND CAST(region_id AS VARCHAR) like :region_id) OR region_id = 0 OR branch_id=0) AND status <> 'deleted') AND status <> 'deleted') " +
    ...
    @Query(value = "SELECT * from cms_2.device where status <> 'deleted' AND device_unique_id =:device_unique_id", nativeQuery = true)
    List<Device> checkDeviceUniqueId(@Param("device_unique_id") String device_unique_id);

    @Modifying
    @Query(value = "UPDATE cms_2.device SET license_key =:license_key,updated_by=:updated_by,updated_date=current_timestamp WHERE device_id =:device_id", nativeQuery = true)
    ...
    @Modifying
    @Query(value = "UPDATE cms_2.device SET device_unique_id =:device_unique_id,updated_by='SYSTEM',updated_date=current_timestamp WHERE device_id =:device_id", nativeQuery = true)
