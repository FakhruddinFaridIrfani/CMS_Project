...
public interface CompanyRepository extends JpaRepository<Company, Integer> {
    ...
    @Query(value = "SELECT * from cms_2.company WHERE " +
...
//              @Param("company_email") String company_email, @Param("created_by") String created_by);
    ...
    @Query(value = "SELECT * from cms_2.company WHERE lower(company_name) = lower(:company_name) AND status not in ('deleted')", nativeQuery = true)
    List<Company> getCompanyByName(@Param("company_name") String company_name);

    @Query(value = "SELECT * from cms_2.company WHERE lower(company_name) = lower(:company_name) AND company_id not in (:company_id) AND status not in ('deleted')", nativeQuery = true)
    List<Company> getCompanyByNameExceptId(@Param("company_name") String company_name, @Param("company_id") int company_id);

    @Modifying
    @Query(value = "UPDATE cms_2.company SET company_name=:company_name,company_address=:company_address,company_phone=:company_phone ,company_email=:company_email," +
    ...
    @Modifying
    @Query(value = "UPDATE cms_2.company SET status = 'deleted',updated_by=:updated_by,updated_date=current_timestamp WHERE company_id=:company_id", nativeQuery = true)
