...
//    void save(@Param("playlist_id") int playlist_id, @Param("resource_id") int resource_id, @Param("order") int order, @Param("created_by") String created_by);
    ...
    @Query(value = "SELECT * from cms_2.Playlist_Resource WHERE playlist_id =:playlist_id AND position_id = :position_id", nativeQuery = true)
    List<PlaylistResource> getPlaylistResourceByPlaylistAndPosition(@Param("playlist_id") int playlist_id, @Param("position_id") int position_id);
