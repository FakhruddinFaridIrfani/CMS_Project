...
public interface RunningTextRepository extends JpaRepository<RunningText, Integer> {
    ...
    @Query(value = "SELECT * FROM cms.running_text WHERE running_text_id =:running_text_id", nativeQuery = true)
    List<RunningText> getRunningTextById(@Param("running_text_id") int running_text_id);

    @Query(value = "SELECT * FROM cms.running_text WHERE branch_id =:branch_id AND status not in ('deleted')", nativeQuery = true)
    List<RunningText> getRunningTextByBranchId(@Param("branch_id") int branch_id);

    @Query(value = "SELECT * FROM cms.running_text WHERE lower(tittle) =lower(:tittle) AND status not in ('deleted') AND branch_id =:branch_id AND region_id=:region_id AND company_id =:company_id", nativeQuery = true)
    ...
    @Query(value = "SELECT * FROM cms.running_text WHERE lower(tittle) =lower(:tittle) AND running_text_id not in (:running_text_id) AND status not in ('deleted') AND branch_id =:branch_id AND region_id=:region_id AND company_id =:company_id", nativeQuery = true)
    ...
    @Modifying
    @Query(value = "UPDATE cms.running_text SET branch_id=:branch_id,region_id=:region_id,company_id=:company_id,tittle=:tittle,description=:description,running_text=:running_text," +
    ...
    @Query(value = "SELECT * FROM cms.running_text WHERE end_date < current_timestamp AND status = 'active'", nativeQuery = true)
